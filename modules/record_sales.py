...
def _available_coupons_df():
    """ตารางคูปองที่ยังใช้ได้ (status ว่าง/active) เพื่อแสดงให้พนักงานดูก่อนกรอก"""
    try:
...
def _render_form(branch_id, sale_date_str, existing):
    ...
    if room > 0:
        ...
        slip_files = None
        st.caption(f"🧾 แนบสลิปครบ {MAX_SLIPS} รูปแล้ว")

    st.divider()
    # ── คูปอง ──────────────────────────────────────────────
    st.markdown("**🎟️ เงินจากคูปอง** — พิมพ์เลขคูปอง (บรรทัดละ 1 เลข)")
    # แสดงข้อมูลคูปองที่ใช้ได้ (ก่อนพิมพ์เลขคูปอง) เพื่อให้พนักงานตรวจดูได้
    _av = _available_coupons_df()
    with st.expander(f"📋 ดูคูปองที่ใช้ได้ในระบบ ({len(_av)} ใบ)",
                     expanded=False):
        if _av.empty:
            st.caption("— ยังไม่มีคูปองที่ใช้ได้ (สำนักงาน/ฝ่ายบัญชีเป็นผู้เพิ่มคูปอง) —")
        else:
            st.dataframe(_av, use_container_width=True, hide_index=True)
    ...
    if save_clicked:
        if coupon_text.strip() and coupon_errors:
            ...
            return
        # ── ตีแป้ง: แป้งสำเร็จรูป ต้องเท่ากับ ส่วนผสม (ถ้าไม่เท่า ต้องกรอกเหตุผล) ──
        if leftover.get("_batter_mismatch") and not leftover.get("batter_mismatch_reason"):
            st.error(
                "❌ จำนวน **แป้งสำเร็จรูป** กับ **ส่วนผสม** ยังไม่เท่ากัน\n\n"
                "กรุณาใส่ **จำนวนที่เท่ากัน** หรือกรอก **เหตุผล** ในช่อง "
                "\"เพราะอะไรถึงไม่เท่ากัน\" ก่อนบันทึก")
            return
        ...
        try:
            ...
            st.error(
                "❌ บันทึกไม่สำเร็จ — ระบบฐานข้อมูลยังไม่พร้อม\n\n"
                "**วิธีแก้:** เปิด Supabase → SQL Editor → รันไฟล์ "
                "`roon_new_tables.sql` (คอลัมน์ใหม่ เช่น แป้งสำเร็จรูป/ส่วนผสม/"
                "เหตุผล ยังไม่มีในตาราง) แล้วลองบันทึกอีกครั้ง\n\n"
...
def _leftover_inputs(prefix, defaults=None):
    ...
    small_mismatch = vals.get("flour_finished_small_used", 0) != vals.get("mix_small_used", 0)
    mismatch = big_mismatch or small_mismatch
    reason = ""
    if mismatch:
        st.warning(
            "⚠️ จำนวน **แป้งสำเร็จรูป** กับ จำนวน **ส่วนผสม** ต้องเท่ากัน\n\n"
            + ("- ถุงใหญ่: แป้งสำเร็จรูป "
               f"{_int(vals.get('flour_finished_big_used',0))} ≠ ส่วนผสม "
               f"{_int(vals.get('mix_big_used',0))}\n" if big_mismatch else "")
            + ("- ถุงเล็ก: แป้งสำเร็จรูป "
               f"{_int(vals.get('flour_finished_small_used',0))} ≠ ส่วนผสม "
               f"{_int(vals.get('mix_small_used',0))}\n" if small_mismatch else "")
        )
        reason = st.text_input(
            "เพราะอะไรถึงไม่เท่ากัน .............................",
            value=str(d.get("batter_mismatch_reason", "")),
            key=f"{prefix}_mismatch_reason",
            placeholder="กรอกเหตุผล — ถ้าไม่กรอก จะบันทึกไม่ได้ จนกว่าจะใส่จำนวนที่เท่ากัน")
    vals["batter_mismatch_reason"] = reason.strip()
    vals["_batter_mismatch"] = mismatch
