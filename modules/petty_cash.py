...
def _get_petty_role() -> str:
    ...
    if dept in ("finance", "accounting"):  return "finance_hq"
    return "branch_staff"
...
def render():
    ...
    if petty_role == "branch_staff":
        # 🏪 สาขา: มีแค่เมนู "บันทึกเบิกเงินสดย่อย" (ตัดเมนูอื่นออกตามที่ ดร.วรรณ กำหนด)
        _render_request_form(petty_role)
    else:
        # ฝ่ายการเงิน HQ / Admin: เห็นเมนูครบ
...
def _petty_branches_dict() -> dict:
    """รายชื่อสาขาที่เชื่อถือได้ — ใช้รายชื่อจาก branch_auth (ต้นฉบับ 20 สาขา) เป็นหลัก
    แล้วเสริมด้วยตาราง branches (ถ้ามีสาขาเพิ่มที่ไม่ซ้ำ)"""
    result = {}
    try:
    ...
    try:
        br_df = read_sheet(SHEET_BRANCHES)
        if not br_df.empty and "branch_id" in br_df.columns and "branch_name" in br_df.columns:
...
def _form_new_request(role: str):
    ...
    branches_dict = _petty_branches_dict()
    locked = str(st.session_state.get("locked_branch_id", "")).strip()

    emp_df = read_sheet(SHEET_EMPLOYEES)
    is_office = (role != "branch_staff")

    if is_office:
        # ── สำนักงาน (office): เลือกพนักงาน office โดยตรง — ไม่ต้องเลือกสาขา ──
        st.markdown("#### ① เลือกพนักงาน (สำนักงาน)")
        st.caption("แสดงพนักงานสำนักงานทุกคน (ตำแหน่งที่ไม่ใช่ 'พนักงานสาขา') ที่สถานะ active")
        pool = emp_df.copy() if emp_df is not None else pd.DataFrame()
        if not pool.empty and "position" in pool.columns:
            pool = pool[~pool["position"].astype(str).str.contains("พนักงานสาขา", na=False)]
        if not pool.empty and "status" in pool.columns:
            pool = pool[pool["status"].astype(str).str.strip().str.lower() == "active"]
        if pool.empty:
            st.warning("⚠️ ไม่พบพนักงานสำนักงาน (office) ที่สถานะ active — กรุณาเพิ่ม/ตรวจที่เมนู HR")
            return
        branch_emps = pool
        sel_branch_id, sel_branch_name = "OFFICE", "สำนักงานใหญ่"
    else:
        # ── พนักงานสาขา: ล็อกเฉพาะสาขาตัวเอง ──
        st.markdown("#### ① สาขา")
        sel_branch_id = locked
        sel_branch_name = branches_dict.get(locked, locked)
        st.info(f"🏪 สาขาของคุณ: {sel_branch_id} – {sel_branch_name} (ล็อกอัตโนมัติ)")
        ...
        if emp_df is not None and not emp_df.empty:
            ...
            branch_emps = emp_df[mask_id | mask_name | mask_name2].copy()
    ...
    st.markdown(
        ...
        f"<tr><td style='color:#000000;padding:4px 8px;font-weight:600;'>💼 ตำแหน่ง</td>"
        f"<td style='color:#000000;padding:4px 8px;'>{str(emp_row.get('position','')).strip() or '-'}</td></tr>"
    ...
    approver_dept = ""   # ตัดแผนกผู้อนุมัติออก — ไม่ต้องรออนุมัติ (ปิดยอดโอนภายในวัน)
