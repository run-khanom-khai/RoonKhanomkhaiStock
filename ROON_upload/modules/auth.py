...
DEPT_MENU_ACCESS = {
    "all":          ["master_data","hr","production","purchase_stock","material_daily",
                     "branch_report","audit","finance","accounting","petty_cash",
                     "dashboard","export","clear_data"],
    "master_data":  ["master_data","export"],
    "hr":           ["hr","export"],
    "production":   ["production","purchase_stock","material_daily","export"],
    "purchase_stock":["purchase_stock","material_daily","export"],
    "branch_report":["branch_report","petty_cash","export"],
    "audit":        ["audit","branch_report","dashboard","export"],
...
def _get_dept_df():
    """ดึงข้อมูล dept — ถ้า Sheet ยังไม่พร้อม ใช้ DEFAULT_DEPTS แทน"""
    try:
        _init_auth_sheet()
        df = read_sheet(SHEET_AUTH)
        ...
        pass
    # Fallback: สร้าง DataFrame จาก DEFAULT_DEPTS
    import pandas as pd
    rows = []
    for dept_id, info in DEFAULT_DEPTS.items():
        rows.append({
...
def render_login(logo_b64: str = "",
    ...
    st.markdown(
        f"<h1 style='text-align:center;color:#FF6B35;font-size:2.4rem;"
        f"font-weight:900;margin:6px 0 0;'>{app_title}</h1>"
        f"<p style='text-align:center;color:#888;margin-top:2px;'>{subtitle}</p>",
        unsafe_allow_html=True,
    )
...
def render_manage_passwords():
    ...
    st.divider()
    st.markdown(
        "<h2 style='color:#E65100;font-size:1.4rem;font-weight:800;"
        "border-left:6px solid #FF6B35;padding-left:12px;'>"
        "🏪 จัดการรหัสผ่านสาขา (แอปสาขา)</h2>",
        unsafe_allow_html=True,
    )
    st.caption("เปลี่ยนรหัสผ่านเข้าแอปสาขาได้ที่นี่ — เช่น เมื่อพนักงานสาขาลาออก "
               "ควรเปลี่ยนรหัสทันทีเพื่อความปลอดภัย")

    try:
        from modules.branch_auth import set_branch_password, branch_login_options
        br_opts = branch_login_options()
    except Exception as e:
        st.error(f"ไม่สามารถโหลดข้อมูลสาขาได้: {e}")
        return

    if not br_opts:
        st.warning("ยังไม่มีข้อมูลรหัสสาขาในระบบ")
        return

    import random as _random
    with st.form("form_change_branch_pw"):
        bc1, bc2, bc3 = st.columns([2, 1, 1])
        with bc1:
            sel_branch = st.selectbox(
                "เลือกสาขา", list(br_opts.keys()),
                format_func=lambda k: f"{k} – {br_opts[k]}")
        with bc2:
            br_new = st.text_input("รหัสผ่านใหม่ (6 หลัก) *", max_chars=6,
                                   placeholder="เช่น 482915")
        with bc3:
            br_confirm = st.text_input("ยืนยันรหัสใหม่ *", max_chars=6)
        cgen1, cgen2 = st.columns(2)
        gen = cgen1.form_submit_button("🎲 สุ่มรหัสใหม่ให้")
        save_br = cgen2.form_submit_button("💾 บันทึกรหัสสาขา", type="primary")

    if gen:
        # ใช้เลขจากเวลาปัจจุบันเป็น seed (ไม่ใช้ในการเข้ารหัส แค่ช่วยสุ่มเลข 6 หลัก)
        _random.seed()
        newcode = str(_random.randint(100000, 999999))
        st.info(f"🎲 รหัสสุ่มที่แนะนำสำหรับสาขา {sel_branch}: **{newcode}** "
                f"— คัดลอกไปกรอกในช่อง 'รหัสผ่านใหม่' แล้วกดบันทึก")

    if save_br:
        if not br_new or not br_new.isdigit() or len(br_new) != 6:
            st.error("รหัสผ่านสาขาต้องเป็นตัวเลข 6 หลัก")
        elif br_new != br_confirm:
            st.error("รหัสผ่านไม่ตรงกัน")
        else:
            if set_branch_password(sel_branch, br_new):
                st.success(f"✅ เปลี่ยนรหัสผ่านสาขา {sel_branch} – "
                           f"{br_opts.get(sel_branch,'')} สำเร็จ "
                           f"(รหัสใหม่: {br_new})")
                st.caption("⚠️ อย่าลืมแจ้งรหัสใหม่ให้สาขา และอัปเดตคู่มือรหัสผ่านของผู้บริหาร")
            else:
