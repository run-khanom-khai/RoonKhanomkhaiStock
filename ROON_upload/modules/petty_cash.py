...
def _form_new_request(role: str):
    ...
    # ── ① เลือกสาขา — แสดงสาขาทั้งหมด หรือพิมพ์รหัสสาขาเองได้ ──
    st.markdown("#### ① เลือกสาขา")

    all_branch_ids = br_df["branch_id"].astype(str).tolist()
    # ค่าเริ่มต้น = สาขาที่ login เข้ามา (ถ้ามี)
    default_code = str(st.session_state.get("locked_branch_id", "")).strip()
    default_idx = all_branch_ids.index(default_code) if default_code in all_branch_ids else 0

    typed_code = st.text_input(
        "พิมพ์รหัสสาขา (ถ้ารู้ เช่น BR004) — หรือเลือกจากรายการด้านล่าง",
        value="", key="req_branch_typed").strip().upper()

    if typed_code and typed_code in all_branch_ids:
        sel_branch_id = typed_code
        st.success(f"✅ ใช้รหัสที่พิมพ์: {sel_branch_id} – "
                   f"{branches_dict.get(sel_branch_id, '')}")
    else:
        if typed_code and typed_code not in all_branch_ids:
            st.warning(f"⚠️ ไม่พบรหัสสาขา '{typed_code}' — กรุณาเลือกจากรายการด้านล่าง")
        sel_branch_id = st.selectbox(
            "สาขา *", all_branch_ids, index=default_idx,
    ...
    if not emp_df.empty:
        ...
        # แสดงพนักงาน "ทุกคน" ในสาขานั้น (ไม่กรองสถานะออก ตามที่ ดร.วรรณ กำหนด)
