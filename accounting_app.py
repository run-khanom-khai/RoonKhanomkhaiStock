"""
accounting_app.py  –  ROON KHANOMKHAI Management System (เวอร์ชันฝ่ายบัญชี)
ผู้พัฒนา: ดร.วรรณ (ดร.อภิวรรณ์ ดำแสงสวัสดิ์) | Copyright © 2026

หน้าที่ของไฟล์นี้:
  - เป็นแอปแยกสำหรับ "ฝ่ายบัญชี (Accounting)" (คนละ URL กับแอปอื่น)
  - ฝ่ายบัญชีทำหน้าที่แทน HR ด้วย
  - Login ด้วยแผนก (dept) — อนุญาตเฉพาะ ฝ่ายบัญชี และผู้ดูแลระบบ (admin)
  - เมนู: Master Data (สาขา+สินค้าสำเร็จรูป) / HR / การเงินและบัญชี /
          Marketing & Sales Reconcile / เงินสดย่อย
...
st.set_page_config(
    page_title="รุนขนมไข่ – ฝ่ายบัญชี (Accounting)",
    page_icon="📒",
    layout="wide",
)

# สีประจำแอป (เขียวเข้ม/เทา) — ต่างจากแอปอื่น กันเปิดผิดแอป
THEME_BG     = "#E0F2F1"
THEME_BORDER = "#00695C"
THEME_TEXT   = "#004D40"
...
if not st.session_state["logged_in"]:
    render_login(LOGO_B64, app_title="📒 ระบบหลังบ้าน",
                 subtitle="ระบบหลังบ้าน — กรุณาเข้าสู่ระบบ")
    st.stop()

# แอปนี้อนุญาตเฉพาะฝ่ายบัญชีและผู้ดูแลระบบเท่านั้น
if st.session_state.get("dept_id") not in {"accounting", "admin"}:
    st.error("⛔ บัญชีนี้ไม่มีสิทธิ์ใช้งานระบบฝ่ายบัญชี")
    st.caption("กรุณาเข้าสู่ระบบด้วยบัญชีฝ่ายบัญชีหรือ Admin")
    if st.button("🚪 ออกจากระบบ", use_container_width=True, key="acc_access_logout"):
        _reset_session()
        st.rerun()
    st.stop()
...
with st.sidebar:
    ...
    st.markdown(
        ...
        f"<small style='color:#888;'>ระบบหลังบ้าน</small><br>"
        f"<b style='font-size:1.05rem;color:{THEME_TEXT};'>"
        f"{st.session_state.get('dept_name','')}</b></div>",
        unsafe_allow_html=True,
    )

    selected_label = st.radio("เมนู", list(ACCOUNTING_MENU.keys()),
...
# MAIN
# ══════════════════════════════════════════════════════════════
_module_path, _func_name = ACCOUNTING_MENU[selected_label]
...
st.markdown(
    ...
    "ROON KHANOMKHAI – ระบบฝ่ายบัญชี (Accounting) | "
