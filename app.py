...
NON_PRICE_COLS = ["lineman_box10","lineman_box20","shopee_box10","shopee_box20",
                  "tiktok_box10","tiktok_box20","grab_box10","grab_box20"]
...
def init_db():
    ...
    existing_inv = [r[1] for r in c.execute("PRAGMA table_info(inventory)").fetchall()]
    if "branch_code" not in existing_inv:
        c.execute("ALTER TABLE inventory ADD COLUMN branch_code TEXT NOT NULL DEFAULT '0000'")
    ...
    c.execute("""CREATE TABLE IF NOT EXISTS sales (
        ...
        shopee_box10 REAL NOT NULL DEFAULT 0, shopee_box20 REAL NOT NULL DEFAULT 0,
        tiktok_box10 REAL NOT NULL DEFAULT 0, tiktok_box20 REAL NOT NULL DEFAULT 0,
        grab_box10 REAL NOT NULL DEFAULT 0, grab_box20 REAL NOT NULL DEFAULT 0,
        note TEXT
    )""")
    ...
    for col,td in new_cols.items():
        if col not in existing:
            try: c.execute(f"ALTER TABLE sales ADD COLUMN {col} {td}")
            except: pass
...
def get_stock_balance_by_branch(branch_code):
    ...
    for item in ITEM_TYPES:
        ...
        if len(sales_df)>0 and item in used_map:
            sf = sales_df[sales_df["branch_code"]==branch_code] if len(sales_df)>0 else pd.DataFrame()
...
.lm-box{background:#EFF6FF;border:1px solid #BFDBFE;border-radius:8px;padding:.5rem .8rem;font-size:.82rem;color:#1E40AF;margin:.5rem 0}
...
if menu == "📊 แดชบอร์ด":
    ...
    if len(sales_df)==0:
        ...
        with col2:
            ...
            bal=get_stock_balance_total()
            short_labels=["กล่อง10","ถุง20","ชาไทยสลัชชี่","ชาไทยเย็น","ชาน้ำผึ้ง","ชาน้ำผึ้งสลัช","ถุงหูหิ้ว"]
        ...
        for i,item in enumerate(ITEM_TYPES):
            ...
            with all_cols[i]:
                if dup and entry_kind=="นำเข้า":
                    st.warning(f"ซ้ำ! มีแล้ว {dup[0][4]:,.0f} {unit}")
                short = item.replace("(ขนมไข่ 10 ชิ้น)","10ชิ้น").replace("(ขนมไข่ 20 ชิ้น)","20ชิ้น").replace("ถุงหูหิ้ว ","")
                ...
                if qty>0:
                    ...
                    if dup and entry_kind=="นำเข้า":
                        ...
                        conn.execute("INSERT INTO inventory (entry_date,branch_code,item_type,qty,entry_kind,note) VALUES (?,?,?,?,?,?)",
                            (str(entry_date),branch_inv_code,item,qty,entry_kind,note_inv))
                        saved+=1
            conn.commit();conn.close()
...
# ══════════════════════════════════════
    ...
    with tab1:
        ...
        for col_obj,(dname,itype,ui,pi,dp,uk,pk) in zip([d1,d2,d3,d4],drink_items):
            sv=bal.get(itype,0)
            with col_obj:
                drink_vals[uk]=st.number_input(f"{dname} (เหลือ {sv:,.0f} แก้ว)",
                    min_value=0.0,step=1.0,format="%.0f",key=uk,
                    value=safe_get(prev,ui) if prev else 0.0,
                    disabled=(sv<1))
                if sv<1: st.caption("ไม่มีบรรจุภัณฑ์นี้")
                drink_prices[pk]=st.number_input("ราคา/แก้ว",
                    value=safe_get(prev,pi,dp) if prev else dp,
                    min_value=0.0,step=1.0,key=pk)

        st.markdown("#### 🛒 ถุงหูหิ้ว ROON (คำนวณเงิน)")
        sb1,sb2,_,_=st.columns(4)
        ssb=bal.get("ถุงหูหิ้ว ROON",0)
        with sb1:
            shopbag_used=st.number_input(f"ถุงหูหิ้ว ROON (เหลือ {ssb:,.0f} ใบ)",
                min_value=0.0,step=1.0,format="%.0f",key="u_sb",
                value=safe_get(prev,16) if prev else 0.0,
                disabled=(ssb<1))
            if ssb<1: st.caption("ไม่มีบรรจุภัณฑ์นี้")
        with sb2:
            shopbag_price=st.number_input("ราคา/ใบ",value=safe_get(prev,17,15.0) if prev else 15.0,
                min_value=0.0,step=1.0,key="p_sb")
        ...
        expected=(box10_used*box10_price+box20_used*box20_price+
                  drink_vals["u_tt"]*drink_prices["p_tt"]+
                  drink_vals["u_mk"]*drink_prices["p_mk"]+
                  drink_vals["u_br"]*drink_prices["p_br"]+
                  drink_vals["u_hn"]*drink_prices["p_hn"]+
        ...
        if st.button("💾 บันทึกยอดขาย",use_container_width=True):
            ...
            vals=(actual_cash,box10_used,box20_used,box10_price,box20_price,
                  drink_vals["u_tt"],drink_vals["u_mk"],drink_vals["u_br"],drink_vals["u_hn"],
                  drink_prices["p_tt"],drink_prices["p_mk"],drink_prices["p_br"],drink_prices["p_hn"],
                  shopbag_used,shopbag_price,
                  lineman_box10,lineman_box20,
            ...
            if dup_sale:
                conn.execute("""UPDATE sales SET actual_cash=?,box10_used=?,box20_used=?,box10_price=?,box20_price=?,
                    ...
                    shopee_box10=?,shopee_box20=?,tiktok_box10=?,tiktok_box20=?,
                    grab_box10=?,grab_box20=?,note=? WHERE id=?""",vals+(dup_sale[0][0],))
                ...
                conn.execute("""INSERT INTO sales (sale_date,branch_code,actual_cash,
                    ...
                    shopbag_used,shopbag_price,lineman_box10,lineman_box20,
                    shopee_box10,shopee_box20,tiktok_box10,tiktok_box20,
                    grab_box10,grab_box20,note) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
        ...
        with report_tab1:
            ...
            if len(df)==0:
                ...
                for _,row in df2.sort_values("branch_code").iterrows():
                    ...
                    rows.append({
                        "รหัสสาขา":row["branch_code"],
                        "ชื่อสาขา":BRANCH_MAP.get(row["branch_code"],""),
                        "ยอดเงินสาขาแจ้ง":f"{row['actual_cash']:,.2f}",
                        "ยอดเงินระบบคำนวณ":f"{row['expected']:,.2f}",
                        "ส่วนต่าง":status,
                    })
                ...
                st.download_button("📥 Export Excel",data=buf.getvalue(),
                    file_name=f"money_report_{report_date}.xlsx",
                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
        ...
        with report_tab2:
            ...
            if len(df)==0:
                ...
                for _,row in df.sort_values("branch_code").iterrows():
                    b10=(safe_get(row,"box10_used")+safe_get(row,"lineman_box10")+
                         safe_get(row,"shopee_box10")+safe_get(row,"tiktok_box10")+safe_get(row,"grab_box10"))
                    b20=(safe_get(row,"box20_used")+safe_get(row,"lineman_box20")+
                         safe_get(row,"shopee_box20")+safe_get(row,"tiktok_box20")+safe_get(row,"grab_box20"))
                    drinks=(safe_get(row,"drink_thaitea_used")+safe_get(row,"drink_milky_used")+
                            safe_get(row,"drink_bright_used")+safe_get(row,"drink_honey_used"))
                    sb=safe_get(row,"shopbag_used")
                    rem10=bal_total.get("กล่องใส (ขนมไข่ 10 ชิ้น)",0)
                    rem20=bal_total.get("ถุงกระดาษ (ขนมไข่ 20 ชิ้น)",0)
                    ...
                    rows2.append({
                        "รหัสสาขา":row["branch_code"],
                        "ชื่อสาขา":BRANCH_MAP.get(row["branch_code"],""),
                        f"กล่องใสรวม (เหลือ {rem10:,.0f})":f"{b10:,.0f}",
                        f"ถุงกระดาษรวม (เหลือ {rem20:,.0f})":f"{b20:,.0f}",
                ...
                st.download_button("📥 Export Excel",data=buf2.getvalue(),
                    file_name=f"pkg_report_{report_date}.xlsx",
                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
    ...
    if len(sales_df)==0:
        ...
        with tab2:
            if len(df_day)==0:
                ...
                pkg_labels=["กล่องใสรวม","ถุงกระดาษรวม","ชาไทยสลัชชี่","ชาไทยเย็น","ชาน้ำผึ้งมะนาว","ชาน้ำผึ้งมะนาวสลัชชี่","ถุงหูหิ้ว"]
                ...
                for ci,(_,row) in enumerate(df_day.iterrows()):
                    ...
                    if sum(vals)>0:
                        with pie_cols[ci%len(pie_cols)]:
                            ...
                            fig_p=go.Figure(go.Pie(labels=pkg_labels,values=vals,
                                marker_colors=colors_pie,hole=0.3,textinfo="label+value"))
                            fig_p.update_layout(height=280,margin=dict(t=10,b=10,l=5,r=5),
                                font=dict(family="Sarabun",size=10),showlegend=False,
                ...
                fig_total.update_layout(height=420,font=dict(family="Sarabun"),
                    paper_bgcolor="rgba(0,0,0,0)")
