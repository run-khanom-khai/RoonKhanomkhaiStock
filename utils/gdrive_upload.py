...
def _get_or_create_folder(service, folder_name: str) -> str:
    """ใช้ Folder ID ที่กำหนดไว้ หรือดึงจาก Secrets"""
    # ลองดึงจาก st.secrets ก่อน
    try:
        import streamlit as st
        fid = st.secrets["gdrive"]["folder_id"]
        if fid:
            return fid
    except Exception:
        pass
    ...
    # ใช้ hardcoded ID
    if HARDCODED_FOLDER_ID:
        return HARDCODED_FOLDER_ID

    # fallback: ค้นหาหรือสร้างใหม่
    query = (
        f"name='{folder_name}' "
