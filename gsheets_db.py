    ...
    for attempt in range(3):
        try:
            ...
            if "429" in err or "quota" in err.lower():
                time.sleep(5 * (attempt + 1))  # รอ 5, 10, 15 วินาที
                continue
            raise e
...
def init_workbook():
    """
    สร้าง Sheet ที่ยังไม่มีเท่านั้น
    ใช้ st.session_state เพื่อ init แค่ครั้งแรกต่อ session
    """
    from config import ALL_SHEETS
    import time

    # ตรวจว่า init แล้วใน session นี้หรือยัง
    try:
        import streamlit as st
        if st.session_state.get("_gsheets_initialized"):
            return
    except Exception:
        pass
    ...
    try:
        ...
        for i, sheet_name in enumerate(missing):
            try:
                ...
                if i > 0 and i % 3 == 0:
                    time.sleep(2)  # พัก 2 วินาทีทุก 3 sheets
            except Exception:
                pass
        ...
        try:
            import streamlit as st
            st.session_state["_gsheets_initialized"] = True
        except Exception:
            pass
