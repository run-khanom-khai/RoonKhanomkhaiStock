"""
production_app.py  –  ROON KHANOMKHAI Management System (เวอร์ชันฝ่ายผลิต)
ผู้พัฒนา: ดร.วรรณ (ดร.อภิวรรณ์ ดำแสงสวัสดิ์) | Copyright © 2026

หน้าที่ของไฟล์นี้:
  - เป็นแอปแยกสำหรับ "ฝ่ายผลิต (Production)" (คนละ URL กับแอปอื่น)
  - Login ด้วยแผนก (dept) — อนุญาตเฉพาะ ฝ่ายผลิต และผู้ดูแลระบบ (admin)
  - เมนู: บันทึก/ประวัติ การผลิต (ข้อมูล Batch / ผลผลิต / วัตถุดิบที่ใช้)
...
st.set_page_config(
    page_title="รุนขนมไข่ – ฝ่ายผลิต (Production)",
    page_icon="🏭",
    layout="wide",
)

# สีประจำแอป (เขียว) — ต่างจากแอปสาขา (ส้ม) และแอปตรวจสอบ (น้ำเงิน) กันเปิดผิดแอป
THEME_BG     = "#E8F5E9"
THEME_BORDER = "#2E7D32"
THEME_TEXT   = "#1B5E20"
...
if not st.session_state["logged_in"]:
    render_login(LOGO_B64, app_title="🏭 ระบบฝ่ายผลิต (Production)",
                 subtitle="ฝ่ายผลิต (Production) — กรุณาเข้าสู่ระบบ")
    st.stop()

# แอปนี้อนุญาตเฉพาะฝ่ายผลิตและผู้ดูแลระบบเท่านั้น
if st.session_state.get("dept_id") not in {"production", "admin"}:
    st.error("⛔ บัญชีนี้ไม่มีสิทธิ์ใช้งานระบบฝ่ายผลิต")
    st.caption("กรุณาเข้าสู่ระบบด้วยบัญชีฝ่ายผลิตหรือ Admin")
    if st.button("🚪 ออกจากระบบ", use_container_width=True, key="prod_access_logout"):
        _reset_session()
        st.rerun()
    st.stop()
...
# ── เมนูของฝ่ายผลิต : แก้ตรงนี้ที่เดียวถ้าต้องการเพิ่ม/ลด ──
# รูปแบบ  "ชื่อที่แสดง" : (module, ฟังก์ชัน)
PRODUCTION_MENU = {
    "🏭 บันทึก/ประวัติ การผลิต": ("modules.production", "render"),
}
...
with st.sidebar:
    ...
    st.markdown(
        ...
        f"<small style='color:#888;'>ระบบฝ่ายผลิต (Production)</small><br>"
        f"<b style='font-size:1.05rem;color:{THEME_TEXT};'>"
        f"{st.session_state.get('dept_name','')}</b></div>",
        unsafe_allow_html=True,
    )

    selected_label = st.radio("เมนู", list(PRODUCTION_MENU.keys()),
...
# MAIN
# ══════════════════════════════════════════════════════════════
_module_path, _func_name = PRODUCTION_MENU[selected_label]
...
st.markdown(
    ...
    "ROON KHANOMKHAI – ระบบฝ่ายผลิต (Production) | "
